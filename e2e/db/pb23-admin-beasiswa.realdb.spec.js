 ...
 * Data di-seed langsung ke DB test, aksi dilakukan lewat UI, hasil diverifikasi
 * ke database (status berubah / baris terhapus). Jalankan: npm run test:e2e:db
 */
test.describe('REAL-DB · PB-23 · Kelola Program Beasiswa oleh Admin', () => {
  ...
  test.beforeEach(async ({ context }) => {
    seeder = makeSeeder();
    pendonor = await seeder.pendonor({ statusOrganisasi: `Yayasan ${seeder.uniq('Org')}` });
    token = seeder.uniq('B'); // penanda unik untuk judul beasiswa test ini
  ...
  test('TC-40-01: daftar beasiswa dari DB tampil di tabel admin', async ({ page }) => {
    await seeder.beasiswa({ pendonorId: pendonor.pendonorId, judul: `Beasiswa Pending ${token}`, status: 'pending' });
    await seeder.beasiswa({ pendonorId: pendonor.pendonorId, judul: `Beasiswa Aktif ${token}`, status: 'aktif' });

    await page.goto('/admin/beasiswa');

    await expect(page.locator(`text=Beasiswa Pending ${token}`)).toBeVisible();
    await expect(page.locator(`text=Beasiswa Aktif ${token}`)).toBeVisible();
    await expect(page.locator(`text=${pendonor.statusOrganisasi}`).first()).toBeVisible();
  });
  ...
  test('TC-40-02 & TC-40-03: pencarian & filter status pada data DB', async ({ page }) => {
    ...
    await search.fill(`Pending ${token}`);
    await expect(page.locator(`text=Beasiswa Pending ${token}`)).toBeVisible();
    await expect(page.locator(`text=Beasiswa Aktif ${token}`)).not.toBeVisible();

    // Filter status Aktif
    await search.fill('');
    await page.click('button:has-text("Aktif")');
    await expect(page.locator(`text=Beasiswa Aktif ${token}`)).toBeVisible();
    await expect(page.locator(`text=Beasiswa Pending ${token}`)).not.toBeVisible();
  });

  test('TC-41-01: menyetujui beasiswa → status di DB berubah jadi aktif', async ({ page }) => {
    const beasiswaId = await seeder.beasiswa({ pendonorId: pendonor.pendonorId, judul: `Beasiswa Pending ${token}`, status: 'pending' });
    await page.goto('/admin/beasiswa');

    const row = page.locator(`tr:has-text("Beasiswa Pending ${token}")`);
  ...
  test('TC-41-03: menghapus beasiswa dengan alasan → baris hilang dari DB', async ({ page }) => {
    const beasiswaId = await seeder.beasiswa({ pendonorId: pendonor.pendonorId, judul: `Beasiswa Hapus ${token}`, status: 'aktif' });
    await page.goto('/admin/beasiswa');

    const row = page.locator(`tr:has-text("Beasiswa Hapus ${token}")`);
    ...
    // VERIFIKASI ke DB: baris tidak ada lagi
