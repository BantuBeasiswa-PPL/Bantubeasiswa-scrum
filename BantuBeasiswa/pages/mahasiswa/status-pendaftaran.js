              ...
              {STEPS.map((step, index) => {
                ...
                return (
                  <div key={step.id} className="relative flex flex-col items-center flex-1">
                    ...
                    {!isLast && (
                      <div
                        className={`absolute top-4 left-1/2 w-full h-[2px] transition-colors duration-500 ${isCompleted ? 'bg-blue-600' : 'bg-gray-200'
                          }`}
                        style={{ borderStyle: isCompleted ? 'solid' : 'dashed' }}
                      />
                    )}
                    ...
                    {/* Step Labels */}
                    <div className="text-center">
                      <h3 className={`text-sm ${isActive ? 'font-bold text-blue-700' :
                          isCompleted ? 'font-medium text-gray-800' :
                            'font-medium text-gray-400'
                        }`}>
                        {step.title}
            ...
            {loading ? (
              ...
              <div className="space-y-4 text-sm">
                ...
                <InfoRow
                  label="Status"
                  value={
                    <span className={`font-semibold ${status === 'LULUS' ? 'text-emerald-600' :
                        status === 'DITOLAK' ? 'text-red-600' :
                          'text-blue-600'
                      }`}>
                      {status ?? '–'}
