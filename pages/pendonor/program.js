...
export default function KelolaProgramPage({ user }) {
  ...
  const handlePublish = async (beasiswa) => {
    ...
    if (beasiswa.status !== 'draft') {
      ...
      await showError('Gagal', 'Hanya program dengan status "Draft" yang dapat dipublish.');
    ...
    const result = await showConfirm(
      ...
      `Publish program "${beasiswa.judul}"? Status akan berubah dari Draft menjadi Aktif.`,
    ...
    try {
      ...
      await showSuccess('Berhasil!', 'Program beasiswa berhasil dipublish.');
  ...
  const handleSubmitApproval = async (beasiswa) => {
    ...
    try {
      ...
      await showSuccess('Berhasil!', 'Program beasiswa berhasil diajukan dan sedang menunggu review admin.');
